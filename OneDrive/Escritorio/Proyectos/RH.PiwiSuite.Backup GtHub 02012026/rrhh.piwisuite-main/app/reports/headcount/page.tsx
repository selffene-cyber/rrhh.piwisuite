...
import { ReportFilters, HeadcountReportRow, HeadcountSummary } from '@/types'
...
import { FaFilePdf, FaFileExcel, FaArrowLeft, FaDownload } from 'react-icons/fa'
...
export default function HeadcountReportPage() {
  ...
  const [rows, setRows] = useState<HeadcountReportRow[]>([])
  ...
  const loadReport = async () => {
    ...
    setLoading(true)
    try {
      const result = await getHeadcountReport(filters, supabase)
      setRows(result.rows)
  ...
  const handleExportPDF = async () => {
    setExporting(true)
    try {
      const response = await fetch('/api/reports/headcount/export-pdf', {
        method: 'POST',
      ...
      link.href = url
      link.download = `Reporte_Dotacion_${new Date().toISOString().split('T')[0]}.pdf`
  ...
  const handleExportCSV = async () => {
    setExporting(true)
    try {
      const response = await fetch('/api/reports/headcount/export-csv', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ filters }),
      })
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = `Reporte_Dotacion_${new Date().toISOString().split('T')[0]}.csv`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '28px', fontWeight: '700', color: '#111827', margin: 0 }}>
            Reporte de Dotación y Distribución
          </h1>
        </div>
      ...
      {summary && (
        <div className="card" style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            <div>
              <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>Total Trabajadores</div>
              <div style={{ fontSize: '24px', fontWeight: '600' }}>{summary.totalEmployees}</div>
            </div>
            <div>
              <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>Centros de Costo</div>
              <div style={{ fontSize: '24px', fontWeight: '600' }}>{summary.byCostCenter.length}</div>
            </div>
            <div>
              <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>AFPs Diferentes</div>
              <div style={{ fontSize: '24px', fontWeight: '600' }}>{summary.byAFP.length}</div>
            </div>
            <div>
              <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>Sistemas de Salud</div>
              <div style={{ fontSize: '24px', fontWeight: '600' }}>{summary.byHealthSystem.length}</div>
            </div>
          </div>
        </div>
      )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                  ...
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px' }}>AFP</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px' }}>Sistema Salud</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px' }}>Tipo Contrato</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px' }}>Estado</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px' }}>Fecha Ingreso</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, index) => (
                  <tr key={index} style={{ borderBottom: '1px solid #e5e7eb' }}>
                    ...
                    <td style={{ padding: '12px', fontSize: '14px' }}>{row.afp || '-'}</td>
                    <td style={{ padding: '12px', fontSize: '14px' }}>{row.health_system || '-'}</td>
                    <td style={{ padding: '12px', fontSize: '14px' }}>{row.contract_type || '-'}</td>
                    <td style={{ padding: '12px', fontSize: '14px' }}>
                      <span className="badge" style={{ 
                        backgroundColor: row.status === 'active' ? '#10b98120' : '#6b728020',
