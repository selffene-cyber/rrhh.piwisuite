...
/**
 * Calcula el tipo de alerta basado en días acumulados y períodos
 */
function calculateVacationAlertType(
  ...
  // CRÍTICO: Tiene 2 períodos completos (60 días) o está cerca de perder días
  if (totalAccumulated >= 60 && periodsCount >= 2) {
    return {
      alertType: 'critical_loss',
      priority: 1,
      message: `¡CRÍTICO! Trabajador con ${totalAccumulated.toFixed(2)} días acumulados (${periodsCount} períodos). Puede perder días si no toma vacaciones pronto.`,
      legalReference: 'Art. 70 Código del Trabajo - Máximo 2 períodos acumulados'
    }
  }
  
  // URGENTE: Más de 45 días acumulados (cerca del límite de 60)
  if (totalAccumulated >= 45) {
    return {
      alertType: 'high_accumulation',
      priority: 2,
      message: `Trabajador con ${totalAccumulated.toFixed(2)} días acumulados. Planificar vacaciones pronto para evitar pérdida.`,
      legalReference: 'Ord. N°6287/2017 DT - Obligación de otorgar feriado antes de nuevo período'
    }
  }
  
  // MODERADO: Más de 30 días acumulados (1 período completo + excedente)
  if (totalAccumulated >= 30) {
    return {
      alertType: 'moderate_accumulation',
      priority: 3,
      message: `Trabajador con ${totalAccumulated.toFixed(2)} días acumulados. Considerar programación de vacaciones.`,
...
export async function getVacationNotifications(
  ...
  try {
    ...
    // Ordenar por prioridad (crítico primero) y luego por días acumulados (mayor primero)
