...
export default function EditDisciplinaryActionPage({
  ...
  useEffect(() => {
    if (currentCompany && params.id && params.actionId) {
      loadData()
    }
  }, [currentCompany, params.id, params.actionId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Error al actualizar amonestación')
      }

      alert('Amonestación actualizada correctamente')
      router.push(`/employees/${params.id}/disciplinary-actions/${params.actionId}`)
    } catch (error: any) {
      alert('Error al actualizar amonestación: ' + error.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1>Editar Amonestación</h1>
        <Link href={`/employees/${params.id}/disciplinary-actions/${params.actionId}`}>
