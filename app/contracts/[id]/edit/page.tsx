...
export default function EditContractPage() {
  ...
  useEffect(() => {
    if (employee && company && formData.clauses.length === 0) {
    ...
    try {
      ...
      setFormData({
        ...
        account_number: contractData.account_number || '',
         ...
         clauses: contractData.clauses && Array.isArray(contractData.clauses) && contractData.clauses.length > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.schedule_regime === 'excluded_art22' && !art22Confirmed) {
        alert('Debe confirmar los requisitos legales del Art. 22 inciso 2 antes de guardar')
        setSaving(false)
        return
      }
          ...
          {formData.schedule_regime !== 'excluded_art22' && (
            <>
              ...
              {(() => {
                ...
                let maxHours = 45
                if (currentYear >= 2028) maxHours = 40
                else if (currentYear >= 2026) maxHours = 42
                else if (currentYear >= 2024) maxHours = 44
                    ...
                    <div style={{
                      ...
                      {!isCompliant && (
                        <div style={{ fontSize: '13px', color: '#991b1b', marginTop: '8px', fontWeight: 'bold' }}>
                          ⚠ ADVERTENCIA: Se excede el límite legal en {hoursDiff.toFixed(2)} horas.
                        </div>
                      )}
