    ...
    if (body.employee_id) {
      ...
      if (!isSuperAdmin) {
        const { data: permissions } = await supabase
          ...
          .select('can_create_permissions')
          .eq('user_id', user.id)
          .eq('company_id', employee.company_id)
          .single()

        if (!permissions?.can_create_permissions) {
          return NextResponse.json(
            { error: 'No tienes permiso para crear permisos' },
            { status: 403 }
          )
        }
      }
      ...
      const validation = await employeeValidator.canCreatePermission(body.employee_id)
