...
export async function GET(request: NextRequest) {
  try {
    ...
    // Por defecto mostrar solo activos para el organigrama
    const status = (searchParams.get('status') as 'active' | 'inactive' | 'all') || 'active'
    const includeEmployeeCount = true // Siempre incluir conteo de empleados para el chart
    ...
    console.error('Error al obtener árbol de departamentos para chart:', error)
    return NextResponse.json(
