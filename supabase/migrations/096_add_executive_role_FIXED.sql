-- ============================================================
-- Migración 096 CORREGIDA: Agregar Rol "Executive" y Sistema de Permisos
-- ============================================================
...
DO $$
BEGIN
  -- Verificar si existe un constraint en user_profiles.role
  IF EXISTS (
    ...
    -- Encontrar y eliminar el constraint
    EXECUTE (
    ...
    RAISE NOTICE '✓ Constraint anterior eliminado de user_profiles.role';
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'No se pudo eliminar constraint de user_profiles.role (puede no existir)';
END$$;
...
-- 1.2 Actualizar company_users.role (opcional, pero recomendado para consistencia)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.constraint_column_usage
    ...
    EXECUTE (
      SELECT 'ALTER TABLE company_users DROP CONSTRAINT ' || constraint_name
      FROM information_schema.table_constraints
      WHERE table_name = 'company_users'
        AND constraint_type = 'CHECK'
        AND constraint_name LIKE '%role%'
    ...
    RAISE NOTICE '✓ Constraint anterior eliminado de company_users.role';
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'No se pudo eliminar constraint de company_users.role (puede no existir)';
END$$;
...
-- Ver constraint actualizado en user_profiles
SELECT 
...
-- Ver constraint actualizado en company_users
SELECT 
  conname AS constraint_name,
  pg_get_constraintdef(oid) AS constraint_definition
FROM pg_constraint
